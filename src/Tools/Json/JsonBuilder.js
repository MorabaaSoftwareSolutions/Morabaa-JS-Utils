...
const Boolean = ({ json, key }) => (
  <span key={key} className='obj-text-1'>{`${json}`}</span>
)
const StringArray = ({ json }) => (
  <span className='obj-text-2 px-sm'>" {json} " , </span>
)
const Image = ({ json }) => (
  <img src={json} style={{ maxWidth: 100, objectFit: 'contain' }} />
)
const ArrayImages = ({ json }) =>
    ...
    <div className='obj'>
      ...
      {json.map((_i, i) => {
        ...
        return _type === 'Object' || _type === 'Array' ? (
          <_Object key={i} json={_i} />
        ) : (
          <StringArray key={i} json={_i} />
        )
      })}
...
const _Object = ({ json }) => {
  ...
  return (
    <div className='json-bg-prim rounded-lg p-sm row-center wrap gap-sm'>
      <span>{'{'}</span>

...
const String = ({ item }: any) => Span({ className: "obj-text-1", innerText: `" ${item} "` });
const Number = ({ item }: any) => Span({ className: "obj-text-1", innerText: `${item}` });
const Boolean = ({ item }: any) => Span({ className: "obj-text-1", innerText: item });
const StringArray = (item: any) => Span({ className: "obj-text-2 px-s", innerText: `"${item}" , ` });
const Image = ({ item }: any) => Img({ style: "max-height: 100px;object-fit: cover;", src: item });
const ArrayImages = ({ item }: any) => item.map((_i: any) => Img({ height: 250, src: _i }));
...
const Array = ({ item }: any) => {
  ...
  return Div({ className: "obj" }, [
    Span({ className: "objtext-3 px-s", innerText: "[" }),
    Fragment(
      item.map((_i: any) => {
    ...
    Span({ className: "obj-text-3 px-s", innerText: "]" }),
  ]);
};
...
const JsonParser = ({ json, setItem, containerClass, onClick }: any) => {
  if (typeof json !== "object") return Span({ className: "json-builder", innerText: json });
